...
function lengthBlackSpacesEndLine(text) {
	let length = 0;
...
function lengthBlackSpacesStartLine(text) {
	let length = 0;
...
function formatNumber(number) {
	const regionCode = global.BlackBot.config.language;
	if (isNaN(number))
...
function getPrefix(threadID) {
	...
	let prefix = global.BlackBot.config.prefix;
...
async function translate(text, lang) {
	...
	for (let i = 0; i < wordTransAfter.length; i++) {
		...
		const numberStartSpace = lengthBlackSpacesStartLine(wordTranslate[i]);
		const numberEndSpace = lengthBlackSpacesEndLine(wordTranslate[i]);
