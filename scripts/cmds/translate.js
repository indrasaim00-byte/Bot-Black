...
module.exports = {
	config: {
		name: "ترجمة",
		aliases: ["trans", "ترجمة", "ترجم", "translate"],
		version: "1.5",
		author: "Saint",
		countDown: 5,
		role: 0,
	...
	onStart: async function ({ message, event, args, threadsData, getLang, commandName }) {
		...
		let content;
		let langCodeTrans;
		const langOfThread = await threadsData.get(event.threadID, "data.lang") || global.BlackBot.config.language;
	...
	onChat: async ({ event, threadsData }) => {
		if (!await threadsData.get(event.threadID, "data.translate.autoTranslateWhenReaction"))
			return;
		global.BlackBot.onReaction.set(event.messageID, {
		...
		switch (Reaction.type) {
			...
			case "translate": {
				...
				if (event.reaction == emojiTrans) {
					const langCodeTrans = await threadsData.get(event.threadID, "data.lang") || global.BlackBot.config.language;
